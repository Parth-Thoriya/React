...
const SearchStu = () =>{
    var text = document.getElementById('stusearch').value;

    var filterStu = stu.filter(
        (student) =>{
            var {Name} = student;
    ...
    return(
        filterStu.map(
          (temp)=>{
              const {Name,Sem,Img} = temp;
              return(
                  <Card title={Name} body={Sem} imgLink={Img} />
              )
          }
        ) 
        );
...
const SearchFac = () =>{
    var text = document.getElementById('facsearch').value;

    var filterFac = temp.filter(
        (fac) =>{
            if(fac[1].toLowerCase().includes(text.toLowerCase())){
                return true;
            }
            return false;
        }
    ...
    return(
        filterFac.map(
            (data)=>{
                ...
                return(
                    <Card title={title} body={body} imgLink={img} />
                )
            }
          ) 
