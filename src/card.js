...
const Card = (fac) => {
  ...
  return (
    <div className="col-3 p-3">
      <div className="card">
        <img src={fac.Image} className="img-fluid" alt="Person" />
        ...
        <Link to={ fac.path + fac.id} className='btn btn-outline-danger'>veiw</Link>
      </div>
    </div>
...
const Stucard = () => {
  ...
  //     Name: "Kishan Dabhi",
  //     Sem: "3rd",
  //     Img: "https://lh3.googleusercontent.com/Ryg5ih-fOWbpEGDpbJYJz5RTq3_28Tvo2h3JQRCdiz16lw5ghitDMN6hcWA57g0d6_8VOqQYcCemRBOfZGQA64ZKkFOn71zuF1LDsXwCh4ywF-_z6VqxbBqmbQvK_-PWfZcztxDd1WDTEEUHX6GdP60"
  //   },
  //   {
  //     Name: "Tejas Parmar",
  //     Sem: "3rd",
  ...
  useEffect(
    ()=>{
      fetch('https://64ded5c171c3335b2581dc88.mockapi.io/Student').then(res => res.json()).then(res => setStu(prev=>res))
      
    }
    , []
  );
  
  let arr = stu.map((temp) => {    
    return (
      <Card id={temp.id} Name={temp.StudentName} Image={temp.StudentImage} body='Semester - 3' path='/student/detail/' />
    )
  });
...
const FacCard = () => {
  ...
  useEffect(
    ()=>{
      fetch('https://64ded5c171c3335b2581dc88.mockapi.io/Faculty').then(res => res.json()).then(res => setTemp(res))
      
    }
    , []
  );
 // console.log('temp', temp);
  let arr = temp.map((fac) => {
    return (
      <Card id={fac.id} Name={fac.FacultyName} Image={fac.FacultyImage} body={fac.Designation} path='/detail/' />
    )
  });
